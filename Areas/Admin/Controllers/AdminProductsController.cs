    ...
    public class AdminProductsController : Controller
    {
        ...
        public IActionResult Index(int page = 1, int CatID = 0)
        {
            ...
            IQueryable<Product> query = _context.Products
                .AsNoTracking()
                .Include(x => x.Cat)
                .OrderByDescending(x => x.ProductId); // Đặt trình tự ban đầu

            if (CatID != 0)
            {
                query = query.Where(x => x.CatId == CatID)
                             .OrderByDescending(x => x.ProductId); // Thêm trình tự nếu cần
            }

            var models = new PagedList<Product>(query, page, pageSize);
